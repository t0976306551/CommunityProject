...
public class insert_staple_Activity extends AppCompatActivity {
    EditText name,context,other;
    Button btn_insertImage,btn_insert;
    ImageView setImage;
    String c_id,m_id;
    Bitmap bitmap;
    final int CODE_GALLERY_REQUEST = 999;
    private static String URL_STAPLE ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_insert_staple);
        ...
        imageAdapter = new ImageAdapter(insert_staple_Activity.this,list_data); // 將資料交給adapter
    ...
    private void insertStaple() {
        final String name = this.name.getText().toString().trim();
        final String context = this.context.getText().toString().trim();
        final String otherStaple = this.other.getText().toString().trim();
        if(name .equals("") || context.equals("") || otherStaple.equals("")){
            Toast.makeText(insert_staple_Activity.this,"欄位不可空白",Toast.LENGTH_SHORT).show();
            return;
        }

        if(list_data.size()==0){
            Toast.makeText(insert_staple_Activity.this,"請挑選圖片",Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try{
            datas.put("s_name", name);
            datas.put("s_context",context);
            datas.put("other",otherStaple);
            datas.put("m_id",m_id);
            datas.put("c_id",c_id);
        ...
        urlSetting = new UrlSetting(insert_staple_Activity.this);
        URL_STAPLE = urlSetting.getUrl()+"staple/create";
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, URL_STAPLE, datas, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try{
                    String success = response.getString("success");
                    if(success.equals("1")){
                        Toast.makeText(insert_staple_Activity.this, "新增成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }else{
                        Toast.makeText(insert_staple_Activity.this, "新增失敗", Toast.LENGTH_SHORT).show();
                    }
                }catch (JSONException e){
                    e.printStackTrace();
...
//                    }
    ...
    private void error_dialog(String text){
        Dialog dialog;
        dialog = new Dialog(insert_staple_Activity.this);
